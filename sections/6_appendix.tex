...
%\banjac{Here is sampling of orientations.}
We compared different directions $(\theta_2, \theta_1)$ constraints on the 2-sphere, shown in \figref{orientation-sampling}; a uniform distribution of orientations over $\SO(3)$ (top left), a non-uniform distribution of orientations---created by sampling Euler angles uniformly following the z-y-z convention (top right) both on full 2-sphere coverage, the half-sphere non-uniform sampling (bottom left), and quarter-sphere uniform sampling (bottom right)---that might happen in real cryo-EM acquisitions. 
Their corresponding distribution of distances is shown in \figref{orientation-sampling:distances}.
...
The difference between uniform and non-uniform sampling on full 2-sphere coverage is shown in Euler angles in \figref{distribution-angles} and quaternion parameters in \figref{distribution-quaternions}.

Comparing the performance from \figref{recovery-nonuniform} and \figref{5j0n-noise0-orientation-recovery},b shows that our method is robust to uneven samplings (uniform vs. non-uniform). Therefore, there is no penalty for non-uniform sampling.
...
\begin{figure}[ht!]
    ...
    \begin{minipage}{.65\linewidth}
        ...
        %     \hspace{1em}%
        %     \includegraphics[height=7em]{figures/5a1a-cvg2.pdf}
        %     \caption{Directions sampled from $(\theta_2, \theta_1) \in [0,\frac{\pi}{2}[ \, \times \, [0,2\pi[$, an half of the sphere (left) and from $(\theta_2, \theta_1) \in [0,\frac{\pi}{2}[ \, \times \, [0,\pi[$, a quarter of the sphere.}
        % \end{subfigure}
        % \hfill
...
%         \includegraphics[height=9em]{figures/5j0n-cvg2.pdf}
...
%         \includegraphics[height=9em]{figures/5a1a-cvg2.pdf}
...
\begin{figure}[ht!]
    ...
    \caption{%
        ...
        %\todo{No penalty for non-uniform sampling.}
...
%\lau{Smooth.}\banjac{Looks fine for me, was it already passed through?}
...
We then report the $(d_q,d_p)$ relationship for all pairs in \figref{euclidean-not-robust}, for full 2-sphere directions coverage (\figref{euclidean-not-robust:5j0n-full}) on \texttt{5j0n}, and quarter and full 2-sphere direction coverage (\figref{euclidean-not-robust:5a1a-quarter} and \figref{euclidean-not-robust:5a1a-full}, respectively) on \texttt{5a1a}.
...
The other interesting observation is that the trend of $(d_q,d_p)$ plot of the \texttt{5a1a} (\figref{euclidean-not-robust:5a1a-full}) appears to take symmetric shape of letter \texttt{M} which can be explained with the fact that this protein has intrinsic dihedral (D2) symmetry. Constraining the direction sampling on the quarter sphere (\figref{euclidean-not-robust:5a1a-quarter}) seems to circumvent the symmetry issue.
...
\begin{figure}[ht!]
    ...
    %        The color corresponds to projection pairs that share one projection, \ie, distance between one projection with all other projections.
            ...
            % \banjac{More pairs. The full-cvg for \texttt{5a1a} is added.}
            % \mdeff{Thanks! I think the old kind of plots illustrates it better. Can we have the three conditions in the old style?}
        }\label{fig:euclidean-not-robust}
    \end{minipage}
\end{figure}
...
The search for optimal embedded feature size resulted in \figref{4d-vs-256d-de} that clearly indicates that a space of $n_f=4$ dimensions is insufficient to represent the complexity/variability of projections. It also suggests that $n_f=8$ already achieves the optimum performance.
...
\begin{figure}[ht!]
    ...
    \end{subfigure}
    \caption{%
        The sister CNN architecture, the part of the distance estimation network architecture from \figref{schematic:distance-learning} where we share the weights with another sister CNN.
        ...
        % This output is a scalar value representing the distance between these two images.
        % Last layer converts feature vector to a higher-dimensional vector of size $n_f=512$. This architecture of network was used throughout all experiments. The last layer was different for experiments in \figref{4d-vs-256d-de}.
        ...
        % \banjac{I will update this plot when I find time during the week.}
