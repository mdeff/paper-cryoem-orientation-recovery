...
More recently, the recovery of geometrical information from unknown view tomography of 2D point sources has been proposed~\cite{zehni2019distance}, but the extension to 3D cryo-EM tomography is not straightforward. \todo{Mdeff can you please add a sentence on the Holmes paper.} \banjac{Another solution tackles the estimation of the in-plane rotation and optical parameters of the particle using generative adversarial networks (GANs) preceded by the variational autoencoders (VAE) to first embed the 2D projections to a lower dimension~\cite{miolane2019estimation}}.
