...
\mdeff{Doesn't $\mathbf{t}$ depend on the particle, i.e., on $i$?}
...
\caption{%
    ...
    Observing that distances between orientations constrain them, we aim to \textit{recover the orientations} $\{q_i\}$ from $\{d_q(q_i, q_j)\}$, where the geodesic distance $d_q(q_i, q_j)$ is the angle between orientations $q_i$ and $q_j$.
