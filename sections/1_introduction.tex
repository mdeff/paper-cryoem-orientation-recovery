...
\begin{equation}
    ...
    \p_i = \mathbf{C}_{\boldsymbol\varphi} \mathbf{S}_{\mathbf{t}} \mathbf{P}_{\bth_i} \x + \mathbf{n},
\end{equation}
where $\x \in \R^V$ is the unknown 3D density map~\cite{dimaio_creating_2007} (Coulomb potential).
The operator $\mathbf{P}_{\bth_i}: \R^V \to \R^{n_p}$ is the projection along the orientation $\bth_i$ (i.e., the x-ray transform).
...
The challenge is to reconstruct $\x$ from a set of projections $\{\p_i\}_{i=1}^P$ while ignoring their orientation.
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.35\linewidth}
        \centering
        \includegraphics[width=\linewidth]{geomProj3D}
        \caption{}\label{fig:imaging-geometry}
    \end{subfigure}
    \hspace{5em}
    \begin{subfigure}[t]{0.35\linewidth}
        \centering
        \includegraphics[width=\linewidth]{intuition-method}
        \caption{}\label{fig:intuition-method}
    \end{subfigure}
\caption{%
    % Goals: explain
    ...
    (a) Geometry of the imaging model defined in \eqnref{imaging-model}.
    ...
    Observing that distances between orientations constrain them, we aim to \textit{recover the orientations} $\{q_i\}$ from $\{d_q(q_i, q_j)\}$, where the geodesic distance $d_q(q_i, q_j)$ is the amplitude of the rotation between orientations $q_i$ and $q_j$.
    Observing that projections taken from close orientations look alike, we aim to \textit{estimate the distance} $d_q(q_i, q_j)$ from the projections $(\p_i, \p_j)$.
...
More recently, the recovery of geometrical information from unknown view tomography of 2D point sources has been proposed~\cite{zehni2019distance}, but the extension to 3D cryo-EM tomography is not straightforward yet.
...
\mdeff{How is our method different from previous work? What's its singularity? Why do we expect it to solve the aforementioned issues?}
