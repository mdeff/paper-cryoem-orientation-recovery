...
Our two-step method relies on the SNN model to estimate pairwise distances between unseen projections, followed by the recovery of the orientations from these distances through an appropriate minimization scheme.
%%%
...
In this regard, the performance of the SNN model could be improved in several ways.
...
Second, the training could be improved, perhaps by providing more supervision by separately predicting the differences in direction $(\theta_2,\theta_1)$ and in-plane angle $\theta_3$.
%%%
...
%%%
% Data gains.
Importantly, the SNN would be better trained on a more exhaustive and diverse cryo-EM dataset.
...
In particular, a necessary extension will be to include the effects of the PSF when generating training data and evaluate its impact on the SNN\@. % like we did for shifts and noise
