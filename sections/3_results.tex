...
Finally, note that it is not possible to resolve chirality as opposed projections are mirrored.\footnote{An object is chiral if it cannot be superposed on its mirror image by any combination of translations or rotations.}
%Global orientation is lost by projecting, and chirality is lost by integrating. 
 Hence, we only trained on half coverage.
...
To train the SiameseNN, we used as inputs pairs of projections, and as outputs their respective quaternion distances (calculated from their orientations).
For the training, we split our projection dataset into distinct training, validation, and test sets (\tabref{dataset}).
