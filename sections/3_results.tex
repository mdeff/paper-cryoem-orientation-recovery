...
\lau{Jelena:}The evolution of the training and validation losses for \texttt{5j0n} and \texttt{5a1a} are presented in \figref{losses-siamese-assym} and \figref{losses-siamese-sym}, respectively.  
\lau{To clean:} The results demonstrate that the SiameseNN succeeds at learning a proxy distance for the \texttt{5j0n} dataset, as convergence was reached in about 50 epochs. %\lau{\textit{I would remove time indication} ($\sim$ 50 minutes in the best resource availability setting)}. 
Both validation losses for \texttt{5j0n} and \texttt{5a1a} were around $0.2$.
However, the current SiameseNN architecture (~\apxref{siamese-architecture}) slightly overfits at learning the distance for \texttt{5a1a}. \lau{Explain why (gap between validation and training, not enough training data, etc.).} \lau{\textit{To remove what follows?} This is most likely due to the symmetry of the protein (even though a quarter-sphere coverage was used \lau{Unclear}).
...
As was the case with the Euclidean distance $d_p$, the larger distances $d_q$ were poorly predicted (see plateau for distances higher than~$2.5$ rad).
...
\lau{Jelena (move before in this section):} We then fed the trained SiameseNN with $1,000$ pairs of projections randomly selected from the \texttt{5j0n} testing dataset, and reported the $(d_q,\widehat{d_p})$ relationship of each pair in \figref{losses-siamese} (right side of each subfigure).
...
%we evaluated the performance of the model when different feature distances were used for the SiameseNN. (see \figref{geo-eucl-mlp} and \apxref{feature-distance}). 
