...
\begin{figure}[t]
    ...
    The second and third columns show density maps $\widehat{\x}$ reconstructed from projections $\{ \p_i \}$, and recovered orientations $\{ \widehat{q_i} \}$ (b,e,h) or true orientations $\{ q_i \}$ (c,f,i).
        ...
        The second row (d,e,f) shows the process for noisy projections $\p_i = \mathbf{P}_{\bth_i} \x + \mathbf{n}$, where $\mathbf{n} \sim \mathcal{N}(0, 16\mathbf{I})$ and $\x$ comes from \texttt{5j0n}.
