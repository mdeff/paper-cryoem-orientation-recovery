...
\mdeff{Story: need to know how good we did (without reconstructing the protein). Orientations are up to a global rotation, so best align recovered and true orientations before computing the mean recovery error.}

\todo{Introduce mean recovery error as a good and intuitive performance metric. Define the alignment objective and how it is a special case of the recovery objective.}
...
\subsubsection{Recovery from exact distances}
\label{subsec:5-6-3-sanity-check}

\mdeff{Story: works perfectly despite no convexity guarantee.}
...
\begin{figure}
    ...
    \centering
        ...
        \caption{}
    \end{subfigure}
    \caption{Results of the orientation recovery scheme when using the perfect orientation distances for the asymmetric protein (5j0n).
\textbf{(a)} Evolution of the loss of~\eqref{eq:global-min-problem} during minimization.
\textbf{(b)} Coverage of $\SOThree$ after the orientation recovery from the perfect relative distances.
}
...
\subsubsection{Sensitivity to distance estimation error}
...
\subsubsection{Learned distance}
...
\subsubsection{Sensitivity to perturbed projections}
