...
\begin{figure}
    \begin{minipage}[b]{0.48\linewidth}
        ...
        \begin{subfigure}[b]{0.49\linewidth}
            ...
            \caption{$E_\text{OR}$ on \texttt{5j0n}.}
        \end{subfigure}
        \hfill
        \begin{subfigure}[b]{0.49\linewidth}
        \centering
            \includegraphics[width=\linewidth]{figures/5a1a_perfect_noisy_ar_aa}
            \caption{$E_\text{OR}$ on \texttt{5a1a}.}
        \end{subfigure}
        \caption{
            Orientation recovery from noisy distances.
            The error $E_\text{OR}$ from \eqnref{orientation-recovery-error} is a monotonic function of the distance estimation error represented as the variance $\sigma^2$ of white noise added to the true distances.
            Better distance estimation leads to better orientation recovery.
            Moreover, the loss $L_\text{OR}$ from \eqnref{orientation-recovery} is a good proxy for the error $E_\text{OR}$, allowing us to assess recovery performance even without ground-truth orientations.
