...
From these atomic models, we generated the density maps in Chimera~\cite{pettersen2004ucsf} by fitting a 1\AA\ map for \texttt{5a1a} and a 3.67\AA\ map for \texttt{5j0n}; this gave us a volume of $110 \times 155 \times 199$ voxels for \texttt{5a1a}, and $69 \times 57 \times 75$ voxels for \texttt{5j0n}.
% -sized volume
...
\figref{euclidean-not-robust:5a1a-full} illustrates the problem.
To capture only one of four identical projections of \texttt{5a1a}, we restricted directions to $(\theta_2, \theta_1) \in [0, \pi[ \, \times \, [0, \frac{\pi}{2}[$, a quarter of the sphere as illustrated in \figref{orientation-sampling} (bottom right), for that protein.
...
This treatment of symmetries is incomplete\footnote{\todo{Explain. TODO: discuss together the treatment of symmetries.}} but sufficient for a proof-of-concept.
...
\figref{orientation-sampling:distances} shows that (mostly) uniform distributions of orientations induce distributions of distances that are skewed towards larger distances.
...
While $1,650$ projections were enough to perfectly reconstruct the density maps,\footnote{We obtained an \todo{FSC of 1 across frequencies} by reconstructing from $1,650$ projections and their ground-truth orientations.} our method is not limited by the number of projections as optimization is done per batch.
...
\figref{results:distance-estimation:noise} shows $E_\text{OR} \approx 0.16$ radians ($\approx 9\degree$) for noiseless projections and $E_\text{OR} \approx 0.42$ radians ($\approx 24\degree$) for a more realistic noise variance of $\sigma^2=16$ (with signal-to-noise ratio of -$11.8$ dB). 
...
\begin{figure}[t]
    ...
    \caption{%
        ...
        The FSC curves in \textbf{(e,j)} indicate the resolutions of the densities reconstructed from recovered orientations (w.r.t.\ ground-truth densities, shown in \figref{density-map:5j0n:ground-truth},d).
