...
\begin{figure}[ht!]
    ...
    \begin{minipage}[b]{0.45\linewidth}
        ...
        \begin{subfigure}[b]{0.45\linewidth}
            \centering
            \includegraphics[width=0.8\linewidth]{figures/5a1a_pdb.png}
            \caption*{\texttt{5a1a}}
        \end{subfigure}
        \hfill
        \begin{subfigure}[b]{0.35\linewidth}
            \centering
            \includegraphics[width=0.8\linewidth]{figures/5j0n_pdb.png}
            \caption*{\texttt{5j0n}}
        \end{subfigure}
        \\ \vspace{1em}
        \begin{subfigure}[b]{0.49\linewidth}
            \centering
            \includegraphics[width=0.8\linewidth]{figures/5a1a_5A_.png}
            \caption*{\texttt{5a1a}}
        \end{subfigure}
        %\hfill
        \begin{subfigure}[b]{0.49\linewidth}
            \centering
            \includegraphics[width=0.8\linewidth]{figures/5j0n_5A_.png}
            \caption*{\texttt{5j0n}}
        \end{subfigure}
        \caption{%
            Ground-truth atomic models (up) and 5\AA\ debsity maps (down): the $\beta$-galactosidase (\texttt{5a1a}), and the lambda excision HJ intermediate (\texttt{5j0n}).
...
\todo{Write the range of angles we sampled from for \texttt{5j0n} and \texttt{5a1a}.} Orientations were obtained by uniformly sampling the Euler angles $\bth=(\theta_1,\theta_2,\theta_3) \in [0, \pi[ \times [0, \pi[ \subset [0, \pi[ \times [0, 2\pi[$ for \texttt{5j0n}, and $\bth=(\theta_1,\theta_2,\theta_3) \in [0, \pi[ \times [0, \pi[ \subset [0, \pi[ \times [0, 2\pi[$ for \texttt{5a1a}.
...
To generate the training, validation, and test datasets, we first split the $P$ projections into three distinct sets (\tabref{dataset}).
...
%     \end{subfigure}
%     \caption{
%         The $E_\text{OR}$ from \eqnref{orientation-recovery-error} is a monotonic function of the distance estimation error represented as a variance of white noise $\sigma^2$ added to the true distances.
%         Better distance estimation leads to better orientation recovery.
%         Moreover, the recovery loss $L_\text{OR}$ from \eqnref{orientation-recovery} is a good proxy for the recovery error $E_\text{OR}$, allowing us to assess recovery performance even without ground-truth orientations.
...
For all variances, the $E_\text{OR}$ is reported in \figref{perfect-with-noise-ar-aa} (blue curve).
...
%     \end{subfigure}
%     \caption{
%         The mean orientation recovery error $E_\text{OR}$ from \eqnref{orientation-recovery-error} is a monotonic function of the distance estimation error represented as a variance of white noise $\sigma^2$ added to the true distances.
%         Better distance estimation leads to better orientation recovery.
%         Moreover, the recovery loss $L_\text{OR}$ from \eqnref{orientation-recovery} is a good proxy for the recovery error $E_\text{OR}$, allowing us to assess recovery performance even without ground-truth orientations.
% }
%     \label{fig:perfect-with-noise-ar-aa}
...
Another interesting output of \figref{perfect-with-noise-ar-aa} is that it indicates that $E_\text{OR}$ behaves as a monotonic function of its loss.
...
%, \; \mathbf{n} \sim \mathcal{N}(0, 16\mathbf{I})$}
    ...
    \end{subfigure}
    \caption{%
        $L_\text{DE}$ \eqnref{distance-learning} evaluated on the training and validation datasets during learning/training in \textbf{(a)} and \textbf{(b)}.
