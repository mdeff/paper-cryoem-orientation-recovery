...
\todo{Figures~\ref{fig:5j0n-noise0-orientation-recovery},~\ref{fig:5j0n-noise16-orientation-recovery} and~\ref{fig:5a1a-noise0-orientation-recovery} show that more than $7000$ steps were used. We don't need to give a number and say something like ran until convergence.}
...
\subsection{Orientation recovery and density reconstruction from estimated distances}
...
%Estimating distances lead to recovering better orientations, which should lead to more accurate reconstructions of density maps.
%We checked how estimation of distances translates to a better reconstruction of the density map, not
As a proof-of-concept, we attempted to solve the inverse problem posed by \eqnref{imaging-model} and reconstructed the density map $\x$ from a set of projections $\{ \p_i \}$ and their orientations $\{ \widehat{q_i} \}$ as recovered by our method.
...
%For a protein, we trained our SiameseNN trough \eqnref{distance-learning} on its training set, then recovered orientations on the test set.
...
\todo{More details. What happened otherwise?}
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.42\textwidth}
        ...
        \caption{Orientation recovery loss $L_\text{OR}$ \eqnref{orientation-recovery} and error $E_\text{OR}$ \eqnref{orientation-recovery-error}.}%
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{Orientation recovery (a) and reconstruction of \texttt{5a1a} from noiseless projections $\p_i = \mathbf{P}_{\bth_i} \x$ and true (b) or recovered (c) orientations.}
\end{figure}

\figref{5j0n-noise0-orientation-recovery} shows the recovery of orientations (from estimated distances) from clean projections of \texttt{5j0n}.
...
The reconstructed density map (\figref{5j0n-noise0-reconstruction-recovered}) is \todo{pretty good?} compared to that with true orientations (\figref{5j0n-noise0-reconstruction-true}).
\mdeff{I'm not sure how to qualify the reconstructed proteins.}

As expected from the experiments in \secref{results:distance-estimation:sensitivity}, adding noise (white noise with variance $\sigma^2=16$) worsen the recovered orientations to a mean error of $E_\text{OR} \approx 0.42$ radians ($\approx 24\degree$), which leads to a worse reconstruction (\figref{5j0n-noise16-reconstruction-recovered}).
...
Orientation recovery is slightly more difficult here \todo{because} \mdeff{Symmetries? Something else? What makes \texttt{5a1a} harder than \texttt{5j0n}?}.
...
The orientation recovery reached the error of $L_\text{OR} = 0.0352$ \mdeff{that's the recovery loss, not error right? Could we have the error? It's easier to understand.},
to be compared with
