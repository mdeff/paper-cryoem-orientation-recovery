...
\banjac{Training took about 8.75 hours on a single Tesla K40c GPU\@ or 3.3 hours on a single Tesla T4 GPU\@. There is a multi-GPU support, hence it can be even faster with more GPU-s.}
...
\banjac{Optimization took about 3.75 hours on a single Tesla K40c GPU\@ if the trigger for early stopping was not activated.
...
\banjac{Angle alignment is usually ran on the CPU and it takes around 8 minutes per run.}
%\todo{Jelena: runtime estimation?}
...
\begin{figure}[t]
    ...
    The second and third columns show density maps $\widehat{\x}$ reconstructed from projections $\{ \p_i \}$, and recovered orientations $\{ \widehat{q_i} \}$~(b,e,h) or true orientations $\{ q_i \}$~(c,f,i).
        ...
        \todo{Word subscripts in roman not italic type, e.g., $L_\text{DE,train}$ not $L_{DE,train}$.}
    }
\end{figure}
