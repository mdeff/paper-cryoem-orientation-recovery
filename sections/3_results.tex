...
Finally, we ran the whole pipeline on a synthetic dataset to assess how well orientations could be recovered from distances estimated by the SNN model (\secref{results:orientation-recovery:reconstruction}).
...
%             \caption{1\AA\ density map $\x$ of \texttt{5a1a} and 3.67\AA\ of \texttt{5j0n}.}
...
%         \end{subfigure}
%         \caption{%
%             \texttt{5j0n} example projections ($\mathbf{n} \sim \mathcal{N}(0, 16\mathbf{I})$).
...
Symmetries are problematic when learning distances: two projections can be identical while not originating from the same orientation, which breaks an axiom of proper distance functions (identity of indiscernibles), see \figref{euclidean-not-robust:5a1a-full}.
%\figref{euclidean-not-robust:5a1a-full} illustrates this problem.
To capture only one of four identical projections of \texttt{5a1a}, we restricted directions to $(\theta_2, \theta_1) \in [0, \pi[ \, \times \, [0, \frac{\pi}{2}[$ (a quarter of the sphere, illustrated in \figref{orientation-sampling:directions}).
...
\figref{different-projections} displays samples of projections.
...
While $1,650$ projections were enough to perfectly reconstruct the density maps (as shown in Figures~\ref{fig:5j0n-reconstruction-fsc} and~\ref{fig:5a1a-reconstruction-fsc}), our method is not limited by the number of projections as optimization is done per batch. Optimization settings are described in \apxref{optimization-settings}.
...
%%%
Moreover, we observe that the loss $L_\text{OR}$ from \eqnref{orientation-recovery} is a reliable proxy for $E_\text{OR}$, allowing us to assess recovery performance in the absence of ground-truth orientations. 
%(\ie, when recovering the orientations of real projections).
    ...
    \begin{minipage}[b]{0.48\linewidth}
        ...
        \begin{minipage}{\linewidth}
            ...
            \begin{tabular}{lrrr}
                ...
                \bottomrule
                \vspace{2em}  % TODO: check if this is allowed 
            \end{tabular}
            }\label{tab:dataset}
        \end{minipage}
...
We evaluated the ability of the SNN to learn to approximate the orientation distance $d_q$ through \eqnref{distance-learning}.
...
% Intro and shift.
We first demonstrated that the learning of distances is insensible to off-centering shifts (\figref{results:distance-estimation:shift}), which is expected given the shift invariance built in our SNN (see \secref{method:distance-learning}).
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}[t]{0.47\linewidth}
        ...
        \caption{%
            Learning from shifted projections $\{ \mathbf{S}_{\mathbf{t}_i} \mathbf{P}_{\bth_i} \mathbf{x} \}$, with triangular distribution shift $(t_{i_1}, t_{i_2}, 0)$.
            %Learning is not harder with the projection shift, because shift invariance is built into the convolutional architecture of $\mathcal{G}_w$.
    ...
    \begin{subfigure}[t]{0.47\linewidth}
        ...
        \caption{%
            Learning from noisy projections $\{ \mathbf{P}_{\bth_i} \mathbf{x} + \mathbf{n} \}$, with white noise $\mathbf{n} \sim \mathcal{N}(0, \sigma^2\mathbf{I})$ of increasing $\sigma^2$.
    ...
    \caption{%
        ...
        Learning is not harder as projections get shifted farther whereas the learning is harder as they become noisier, because shift invariance is built into the convolutional architecture of $\mathcal{G}_w$ and noise invariance is not.
...
This is obviously a specific experimental case that only partially shines light on the applicability of the method in real situations; discussed in \secref{discussion}.
...
A mean error of $E_\text{OR} \approx 0.20$ radians ($\approx 11\degree$) in the recovered orientations led to a reconstruction with a resolution of 12.2\AA\ at a Fourier shell coefficient (FSC) of $0.5$, see \figref{5j0n-noise0-reconstruction-recovered}.
...
%%%
Finally, \figref{5a1a-noise0-orientation-recovery},d show the recovery of orientations from noiseless and noisy projections of \texttt{5a1a}.
A mean error of $E_\text{OR} \approx 0.13$ radians ($\approx 7\degree$) in both cases led to reconstructions with resolutions of 8.0\AA\ and 9.6\AA, see \figref{5a1a-noise0-reconstruction-recovered},i.
...
\begin{figure}[t]
    ...
    \caption{%
        ...
        The Fourier shell correlation (FSC) curves in (e,j) indicate the resolutions of the densities reconstructed from recovered orientations (w.r.t.\ ground-truth densities,  \figref{density-map:5j0n:ground-truth},d).
