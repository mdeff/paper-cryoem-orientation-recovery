...
\begin{figure}[t]
    ...
    \label{fig:noise-orientation-recovery}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{%
        ...
        \mdeff{Shall we show the densities reconstructed from random orientations? If not, we could show our reconstructed densities in Figure 11. The ground-truths are not exactly the ones in Figure 6, but they should be right?} \lau{There is some display issues as (c) and (d) appear much bigger than (a). I think it's great to have the reconstruction from random angles; is that what's illustrated in (b) and (f) right now? MAIN COMMENT: I would put the FSC curves currently in Figure 11 in Figure 12. Also I would remove the reconstructions displayed in Figure 11 as they are redundant with Figure 12. Keep Figure 11 about angle recovery, and Figure 12 about reconstruction.}
    }
    \label{fig:noise-reconstruction}
\end{figure}
