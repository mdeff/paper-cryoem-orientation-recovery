...
As expected from the experiments in \secref{results:distance-estimation:sensitivity}, adding noise (white noise with variance $\sigma^2=16$) worsen the recovered orientations (\figref{5j0n-noise16-orientation-recovery}) to a mean error of $E_\text{OR} \approx 0.42$ radians ($\approx 24\degree$), which leads to a worse reconstruction (\figref{5j0n-noise16-reconstruction-recovered}).
