...
%     \item \todo{use $\bth=(\theta_3,\theta_2,\theta_1)$}
...
It was hence sufficient to only sample half the directions \banjac{$(\theta_2, \theta_1) \in [0, \pi[ \, \times \, [0, \pi[ $} (\figref{imaging-geometry}). % for \texttt{5j0n}
...
\banjac{I will rerun that again. In the log I found that we started using half-coverage when working with kNN and Euclidean distance. I don't remember if I ran on the full coverage after, maybe, but I will reran again to be sure.}
\mdeff{Hmm interesting\ldots. It shouldn't be an issue for Euclidean either. So maybe it was something else. Let's see. Thanks.}
\banjac{Located in Appendix, full coverage with uniform angles. The uniform rotations is running.}
...
\mdeff{Move that discussion to \secref{method:distance-learning} if full coverage works.}
\mdeff{Then mention somewhere that 3.1 (fig 8) and 3.3 (fig 10) were done with half-coverage for historical reasons but that the conclusions wouldn't change. Or even omit it, as they are just intermediate results about the second part of the pipeline which isn't much concerned by coverage.}
%\banjac{Following the intuition from using an Euclidean distance as our $d_p$, we assumed the symmetries will be problematic to learn distances...}
%\banjac{Nevertheless, we tried distance learning and estimation on a full direction coverage of \texttt{5j0n}...}
...
For this reason, we further restricted orientations to \banjac{$(\theta_3,\theta_2,\theta_1) \in [0, 2\pi[ \, \times \, [0, \pi[ \, \times \, [0, \frac{\pi}{2}[$} for \texttt{5a1a} and \banjac{$(\theta_3,\theta_2,\theta_1) \in [0, 2\pi[ \, \times \, [0, \frac{\pi}{2}[ \, \times \, [0, 2\pi[$} for \texttt{5j0n} (\figref{orientation-constraints}).
\mdeff{For consistency, could we restrict the range of only one of $\theta_2$ or $\theta_1$? (Now we restrict $\theta_1$ for \texttt{5j0n} and $\theta_2$ for \texttt{5a1a}.) The idea is to restrict to half in the previous paragraph (for all proteins irrespective of symmetries) then restrict further in this paragraph for symmetric proteins (\ie, for \texttt{5a1a} not \texttt{5j0n}).}
\banjac{I agree. However, I will see how time serves me to fix all the 5a1a experiments.}
...
\todo{Write better. Might change due to the experiment above.}
...
%\mdeff{That's not a uniform sampling of $\SO(3)$. Shall we mention it? Argue for it?}
...
\lau{TO DO.}\mdeff{Laurène: could you write a sentence (with a ref?) about the expected distribution of orientations from a real cryo-EM acquisition? I remember we discussed that $\SO(3)$ uniform was a reasonable model but that deviation from uniformity is to be expected. (That could be a motivation for the choice of Euler uniform: it shows the method works even if orientations aren't uniformly sampled.)}
\mdeff{Jelena: if it's easy, we might want to try recovery and reconstruction from uniformly sampled orientations (maybe it's better?). If it's not easy, we should write something about why we chose Euler uniform. (No need to say it's best. We can say we started with that and a more realistic sampling would be $\SO(3)$ uniform.)}
...
Figures~\ref{fig:uniform-orientations},\ref{fig:uniform-angles}a and~\ref{fig:orientation-constraints}b,d show that a (mostly) uniform distribution of orientations induces a distribution of distances that is skewed towards larger distances.
As this would skew $L_\text{DE}$ and bias $\widehat{d_p}$, we further sample pairs to make the distribution of distances uniform---for $\widehat{d_p}$ to be uniformly accurate over the whole $[0,\pi]$ range of distances.
...
\todo{Write here that half-coverage and Euler uniform in figures 8 and 10 are for historical reasons.}
...
\todo{Move to Appendix with the corresponding figure.}
...
\todo{Reference \apxref{non-determinism}, rerun for robustness to unseen}
...
\begin{figure}[ht!]
    ...
    \caption{%
        ...
        %\mdeff{y-axis could be even smaller than 1: I would use the smallest possible before the boxes obscure the box plots in (b).}
    }
\end{figure}
...
\lau{Decide what we want to put in appendix.} As a proof-of-concept, we attempted to solve the inverse problem posed by \eqnref{imaging-model}.
...
A mean error of $E_\text{OR} \approx 0.16$ radians ($\approx 9\degree$) in the recovered orientations led to the reconstruction shown in \figref{5j0n-noise0-reconstruction-recovered}, to be compared to the reconstruction from true orientations shown in \figref{5j0n-noise0-reconstruction-true}. \lau{Decide what we want to say based on Jelena's latest results.}
...
Unsurprisingly, this leads to a reconstruction with lower resolution, as shown in \figref{5j0n-noise16-reconstruction-recovered}. Note that part of this decrease in resolution originates from reconstructing with degraded projections, as the same effect is observed with the reconstruction obtained from true orientations (\figref{5j0n-noise16-reconstruction-true}).
...
A mean error of $E_\text{OR} \approx 0.19$ radians ($\approx 11\degree$) in the recovered orientations led to the reconstruction shown in \figref{5j0n-noise0-reconstruction-recovered}, to be compared to \figref{5j0n-noise0-reconstruction-true}. \lau{Same here, decide what we want to say.}
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.56\linewidth}
        ...
        \includegraphics[height=10em]{figures/5j0n_fullcvg_uniformS2_noise0_ar_aa.pdf}
    ...
    \begin{subfigure}[b]{0.4\linewidth}
        ...
        \includegraphics[height=10em]{figures/5j0n_fullcvg_uniformS2_noise0_FSC_apr_init.pdf} 
    ...
    %     \centering
    %     \includegraphics[height=8em]{figures/FSC_5j0n_fullcvg_noise0_gt_vs_init.pdf}
    %     \caption{FSC between ground-truth reconstruction and initial volume (goal).}%
    %     \label{fig:5j0n-noise0-reconstruction-recovered}
    % \end{subfigure}
    \\ \vspace{1em}
    \begin{subfigure}[b]{0.56\linewidth}
        ...
        \includegraphics[height=10em]{figures/5j0n_fullcvg_uniformS2_noise16_ar_aa.pdf}
    ...
    \begin{subfigure}[b]{0.4\linewidth}
        ...
        \includegraphics[height=10em]{figures/5j0n_fullcvg_uniformS2_noise16_FSC_apr_init.pdf}
        \caption{FSC between final reconstruction and initial volume.}%
    ...
    % \begin{subfigure}[b]{0.24\linewidth}
    %     \centering
    %     \includegraphics[height=8em]{figures/FSC_5j0n_fullcvg_noise16_init_vs_gt.pdf}
    %     \caption{FSC between ground-truth reconstruction and initial volume (goal).}%
    %     \label{fig:5j0n-noise0-reconstruction-recovered}
    % \end{subfigure}
    \\ \vspace{1em}
    \begin{subfigure}[b]{0.56\linewidth}
        \centering
        \includegraphics[height=10em]{figures/5a1a_quartercov_uniformS2_noise0_halfInplane_ar_aa.pdf}
    ...
    \begin{subfigure}[b]{0.4\linewidth}
        ...
        \includegraphics[height=10em]{figures/r5a1a_quartercov_uniformS2_noise0_halfInplane_FSC_apr_init.pdf}
        \caption{FSC between final reconstruction and initial volume.}%
    ...
    % \begin{subfigure}[b]{0.24\linewidth}
    %     \centering
    %     \includegraphics[height=8em]{figures/FSC_5a1a_gt_vs_init.pdf}
    %     \caption{FSC between ground-truth reconstruction and initial volume (goal).}%
    %     \label{fig:5a1a-noise0-reconstruction-recovered}
    % \end{subfigure}
    
    \caption{%
    \todo{Change caption respectively to content}
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.32\linewidth}
        ...
        \includegraphics[height=8em]{figures/5j0n_fullcvg_uniformS2_noise0_gt.png}
    ...
    \begin{subfigure}[b]{0.32\linewidth}
        ...
        \includegraphics[height=8em]{figures/5j0n_fullcvg_uniformS2_noise0_rand.png}
    ...
    \begin{subfigure}[b]{0.32\linewidth}
        ...
        \includegraphics[height=8em]{figures/5j0n_fullcvg_uniformS2_noise0_apr.png}
        \caption{Final}%
        \label{fig:5j0n-noise0-reconstruction-true}
    \end{subfigure}
    \\ \vspace{1em}
    ...
    \begin{subfigure}[b]{0.32\linewidth}
        \centering
        \includegraphics[height=8em]{figures/5j0n_fullcvg_uniformS2_noise16_apr.png}
        \caption{Final}%
    ...
    \begin{subfigure}[b]{0.32\linewidth}
        \centering
        \includegraphics[height=8em]{figures/r5a1a_quartercov_uniformS2_noise0_halfInplane_gt.png}
        \caption{Ground-truth.}%
        \label{fig:5j0n-noise0-reconstruction-true}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\linewidth}
        \centering
        \includegraphics[height=8em]{figures/r5a1a_quartercov_uniformS2_noise0_halfInplane_rand.png}
        \caption{Random angles.}%
        \label{fig:5a1a-noise0-reconstruction-recovered}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\linewidth}
        \centering
        \includegraphics[height=8em]{figures/r5a1a_quartercov_uniformS2_noise0_halfInplane_apr.png}
        \caption{Final.}%
        \label{fig:5a1a-noise0-reconstruction-true}
    \end{subfigure}
    \caption{%
        \todo{Add reconstructions - 4 images per protein: initial, ground truth, without alignment, aligned}
    }
\end{figure}
