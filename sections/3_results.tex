...
\begin{table}[ht!]
    ...
    \caption{%
        ...
        \banjac{thanks for remark, and I totally agree we need to use combinations of 2, however, I checked (and didn't notice before) that we use $P^2$ :(} \lau{To be settled during today's meeting.}
    }\label{tab:dataset}
\end{table}
...
Orientations were recovered through \eqnref{orientation-recovery} in a stochastic setting, with the loss function varying over the batches. To ensure that orientation recovery was performed on projections unseen during the distance learning, we used the test set from \tabref{dataset}\lau{. A bit unclear which set you're talking about}.

\lau{Unclear: Since \textbf{mean orientation error} is used in the pose estimation tasks and it is considered reliable performance metric, we decided to use it as our performance measure (see \apxref{metrics-review})}. The average difference between predicted and actual angles is given in degree (or rad), which makes it an intuitive comparison metric.
\figref{5j0n-aa-loss-perfect-distances} shows a successful convergence and \textbf{mean orientation recovery error} before and after alignment with the perfect distance $d_q$.

\paragraph{Optimization settings.}

We optimized \eqnref{distance-learning} with the RMSProp optimizer~\cite{tieleman2012rmsprop} and a learning rate of $10^{-3}$ for $150$ epochs. With batch size of $256$, we have $247$ steps per epoch in training set and 28 steps per epoch in validation set (\tabref{dataset}).
...
\mdeff{Above is all about data. Shall we write here about ``optimization settings''? I think they apply to all experiments, but we could clarify if some differ. Alternatively, they could go in the Method section (though to me they are more of a practical than method matter). What do you think?}\banjac{Shall I make pseudo algorithm for the two parts of method? I will check other settings.}
\mdeff{Jelena: could you check the settings, make sure they are used for all experiments, and add anything missing?} \lau{To be settled during today's meeting.}
...
To assess the progress and effectiveness of our distance estimation implementation, we used the Euclidean distance as baseline (see \apxref{results:distance-estimation}).
...
\lau{\textit{Unclear numbers for me:} The pairs for the training were sampled from $1\%$ of maximum number of training pairs $P_{\text{train}}^2$ ($63,126$ pairs) and validation was performed on $1\%$ of the maximum number of validation pairs $P_{\text{val}}^2$ ($27,225$ pairs).} 
\lau{\textit{Necessary}? We restricted the size of our training and validation datasets due to Google Colaboratory\footnote{\lau{A hosted Jupyter notebook service from Google Research with resources that are not guaranteed and not unlimited.}} training time limit of 12 hours.}
...
The training lasted from 2.6 hours to 9.3 hours on one GPU, depending on the available resources on \lau{the Google Colaboratory.} 
\lau{\textit{Unclear}: The evolution of the training and validation losses are presented on the left side in \figref{losses-siamese-assym} for \texttt{5j0n}, and on the left side in \figref{losses-siamese-sym} for \texttt{5a1a}}. 
The results demonstrate that the SiameseNN succeeds at learning a proxy distance for the \texttt{5j0n} dataset, as convergence was reached in about 50 epochs \lau{\textit{I would remove time indication} ($\sim$ 50 minutes in the best resource availability setting)}. \lau{Ok but what about 5a1a at this stage?}

\lau{To clarify}. As was the case with the Euclidean distance $d_p$, the larger distances $d_q$ were poorly predicted (see plateau for distances higher than~$2.5$ rad).
...
Both validation losses for \texttt{5j0n} and \texttt{5a1a} were around $0.2$.
However, the current SiameseNN architecture (\lau{Appendix X}) slightly overfits at learning the distance for \texttt{5a1a}; this is most likely due to the symmetry of the protein (even though a quarter-sphere coverage was used \lau{Unclear}).
...
For the rest of the experiments (\lau{Sections XXX}), we used the \texttt{5j0n} dataset. \lau{Unclear:
...
We then fed the trained SiameseNN with $1,000$ pairs of projections randomly selected from the \texttt{5j0n} testing dataset, and reported the $(d_q,\widehat{d_p})$ relationship of each pair in \figref{losses-siamese} (right side of each subfigure).
...
The prediction performance was slightly better for the \texttt{5j0n} \lau{compared to \texttt{5a1a}. \textit{Confusing, before we say we don't run on 5a1a}}.
...
\lau{\textit{Shouldn't we put that before? }The architecture of implemented SiameseNN can be seen in \apxref{siamese-architecture}.}
Finally, %the hyperparameter selection and tuning of the neural network layers, 
we evaluated the performance of the model when different feature distances were used for the SiameseNN. (see \figref{geo-eucl-mlp} and \apxref{feature-distance}) \lau{Shouldn't we say a short word on the result of this?}
...
\mdeff{How good is that?}\banjac{To be discussed tomorrow}
