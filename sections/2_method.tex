...
\todo{In \secref{experiments}, we refer to this value? as the $L_\text{DE}$ loss.}
...
\todo{After some experimention, we ended up with the architecture described in \apxref{siamese-architecture} (for all experiments). While it gave good performance for a reasonable runtime, it can be optimized further. Especially when training on more data (beyond our proof-of-concept).}
...
\todo{In \secref{experiments}, we refer to this value? as the $L_\text{OR}$ loss.}
...
\end{equation}
\mdeff{$E_\text{OR}$ for consistency with $L_\text{OR}$? Nice for \secref{experiments}.}
...
\mdeff{Why is it good? Intuitive. Laurène, can we say something more?}
