...
\begin{figure}
    ...
    \caption{%
        ...
        \mdeff{Step 2: $q \in \mathbb{S}^3$, not $\mathbb{H}$.}
...
Our two-steps method is illustrated in \figref{schematic:method-overview}.
...
Unit quaternions $q \in \mathbb{S}^3$, where $\mathbb{S}^3 = \big\{ q \in \mathbb{H}: \lvert q \rvert = 1 \big\}$ is the 3-sphere with the additional group structure inherited from quaternion multiplication, concisely and elegantly parameterize rotations as
\begin{equation*}
    \mathbf{R}_q =
    \begin{pmatrix}
...
In practice, \eqnref{distance-learning} is minimized by stochastic gradient descent (SGD) over small batches of pairs, with weight updates computed by back-propagating the error.
